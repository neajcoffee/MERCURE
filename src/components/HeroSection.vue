  ...
  <section class="hero-section">
    ...
    <div class="hero-container">
      <div :class="['hero-content', { 'hero-content--visible': isLoaded }]">
        ...
        <div class="hero-badge">
          Vous êtes e-commerçant ? <svg class="hero-badge-icon" width="30px" height="30px" viewBox="-18 0 292 292"
        ...
        <h1 class="hero-title">
          Transformer votre marque en <strong class="hero-title-highlight">machine à cash
          </strong>
        </h1>
        ...
        <p class="hero-description">
          Le thème Shopify conçu sur la base des boutiques e-commerce les plus rentables, optimisé pour convertir,
          renforcer votre marque et réduire vos coûts.
        </p>
...
.hero-section {
  ...
  background: linear-gradient(0deg, #00000057 15%, transparent 60%);
  position: relative;
  overflow: hidden;
}
...
.marquee-banner {
  ...
  padding: var(--spacing-sm) 0;
  mask: linear-gradient(90deg, #0000 5%, #fff 35% 65%, #0000 95%);
}
...
/* Effet pointilliste de base */
.hero-bg::before {
  content: '';
  position: absolute;
  inset: 0;
  background-image: radial-gradient(var(--landing-color-accent) 0.5px, transparent 1px), radial-gradient(var(--landing-color-accent) 0.5px, transparent 0.8px);
  background-size: 12px 12px, 23px 16px;
  background-position: 11px, 16px, 7px 7px;
  opacity: 0.08;
  mask: linear-gradient(0deg, #0000 15%, #fff 45%);
}

/* Effet pointilliste avec glow */
.hero-bg::after {
  content: '';
  position: absolute;
  inset: 0;
  background-image:
    radial-gradient(var(--landing-color-accent) 0.8px, transparent 1.2px),
    radial-gradient(var(--landing-color-accent) 0.4px, transparent 0.8px);
  background-size: 30px 30px, 40px 40px;
  background-position: 4px 4px, -8px -8px;
  opacity: 0.05;
  filter: blur(1px);
  mix-blend-mode: screen;
  mask: linear-gradient(0deg, #0000 0%, #fff 20%);
}
